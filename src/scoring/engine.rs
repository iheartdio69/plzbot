...
pub fn score_and_manage(
    ...
    for (mint, st) in coins.iter_mut() {
        ...
        // unique signers from events in last 5m
        let cutoff = now.saturating_sub(300);
        let mut signers = std::collections::HashSet::new();

        // keep last 20m of events to cap growth
        st.events.retain(|e| e.ts >= now.saturating_sub(1200));
        ...
        if st.events.is_empty() {
            // no onchain feed yet; don't penalize
        } else if st.unique_signers_5m >= cfg.min_signers_for_target {
            score += 20;
        } else {
            score -= 10;
        }
